...
// NAF rev.2 labels — 732 classes, source: INSEE int_courts_naf_rev_2.xls
const NAF_LABELS = {
...
const total = db.prepare(`
  SELECT COUNT(DISTINCT activitePrincipaleEtablissement) as cnt
  FROM etablissements
  WHERE etatAdministratifEtablissement = 'A'
`).get();
console.log('Total codes NAF distincts:', total.cnt);

const rows = db.prepare(`
  SELECT activitePrincipaleEtablissement as naf, COUNT(*) as cnt
  FROM etablissements
  WHERE etatAdministratifEtablissement = 'A'
  GROUP BY naf
  ORDER BY cnt DESC
...
const data = rows.map(function(x, i) {
  return {
    Rang: i + 1,
    'Code NAF': x.naf,
    'Libellé': NAF_LABELS[x.naf] || '',
    'Nombre établissements': x.cnt,
  };
});

const ws = XLSX.utils.json_to_sheet(data);
ws['!cols'] = [{ wch: 6 }, { wch: 12 }, { wch: 65 }, { wch: 25 }];
const wb = XLSX.utils.book_new();
XLSX.utils.book_append_sheet(wb, ws, 'NAF');
XLSX.writeFile(wb, 'naf-counts.xlsx');
console.log('Exported to naf-counts.xlsx (' + rows.length + ' codes).');